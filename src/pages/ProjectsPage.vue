    ...
    <div class="mx-auto max-w-7xl px-6 py-16">
      ...
      <template v-else>
        <div class="mb-8 flex flex-wrap gap-2">
          <button v-for="opt in filterOptions" :key="opt.value" class="rounded-lg px-4 py-2 text-sm font-medium transition-colors" :class="selectedStatus === opt.value
...
const filteredProjects = computed<Project[]>(() =>
  selectedStatus.value === 'all'
    ? projects.value
    : projects.value.filter((p) => p.status === selectedStatus.value),
)
...
const filterOptions = computed(() => [
  { value: 'all' as StatusFilter, label: 'All', count: projects.value.length },
  { value: 'active' as StatusFilter, label: 'Active', count: projects.value.filter((p) => p.status === 'active').length },
  { value: 'completed' as StatusFilter, label: 'Completed', count: projects.value.filter((p) => p.status === 'completed').length },
  { value: 'future' as StatusFilter, label: 'Future', count: projects.value.filter((p) => p.status === 'future').length },
])
</script>
