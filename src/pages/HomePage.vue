  ...
  <MainLayout>
    <div>
      ...
      <div class="mx-auto max-w-7xl px-6 py-16">
          ...
          <div class="rounded-2xl bg-white p-8 shadow-sm">
            ...
            <div class="mt-6 grid grid-cols-3 gap-4 border-t pt-6">
              <div>
                ...
                <p class="text-lg font-bold text-gray-900">{{ formatCurrency(activeProject.goal) }}</p>
              </div>
              <div>
                <p class="text-xs text-gray-400">Raised</p>
                <p class="text-lg font-bold text-green-700">{{ formatCurrency(activeProject.raised) }}</p>
              </div>
              <div>
                <p class="text-xs text-gray-400">Remaining</p>
                <p class="text-lg font-bold text-gray-900">{{ formatCurrency(activeProject.goal - activeProject.raised) }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mb-16">
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <p class="mt-2 text-sm text-gray-700">{{ update.update }}</p>
            </div>
            <div v-if="latestUpdates.length === 0" class="text-center text-gray-400 py-8">
              No updates yet.
            </div>
          </div>
        </div>
        ...
        <div class="mb-16">
          <SectionHeader title="Project Photos" subtitle="Before &amp; after documentation (photos coming soon)" />
        ...
        <div class="rounded-2xl bg-gradient-to-r from-green-600 to-green-800 p-10 text-center text-white">
          <h2 class="text-2xl font-bold">Support Our Community</h2>
...
const activeProject = computed(() => projects.find((p) => p.status === 'active'))
const totalRaised = computed(() => projects.reduce((sum, p) => sum + p.raised, 0))
